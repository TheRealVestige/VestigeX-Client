...
public final class ItemDef
{
    ...
    public static void itemModel()
    {
        try
        {
            for(int i = 0; i < 20793; i++)
            {
                ...
                byte abyte0121[] = FileOperations.ReadFile((new StringBuilder()).append("./items/").append(itemDef.modelID).append(".mdl").toString());
                FileOperations.WriteFile((new StringBuilder()).append("./dumped/").append(itemDef.modelID).append(".mdl").toString(), abyte0121);
            }
    ...
    private void readValues(Stream stream)
    {
        do
        {
            ...
            if(i == 121)
                lendID_test = stream.readUnsignedWord();
            else
            if(i == 122)
                lendTemplateID_test = stream.readUnsignedWord();
        } while(true);
    }
    ...
    public void toNote()
    {
        ...
        if(c == 'A' || c == 'E' || c == 'I' || c == 'O' || c == 'U')
            s = "an";
        description = (new StringBuilder()).append("Swap this note at any bank for ").append(s).append(" ").append(itemDef_1.name).append(".").toString();
        stackable = true;
    }
